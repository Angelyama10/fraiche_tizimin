...
export function OrderTracking({ orderToken }: { orderToken: string }) {
  ...
  async function openPayment() {
    ...
    try {
      ...
      notify({ title: 'No pudimos abrir el pago', description: errorMessage(error), tone: 'error' });
  ...
  async function uploadProof(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await auth.request(`/payments/orders/${order.publicToken}/transfer-proof`, { method: 'POST', body: JSON.stringify({ objectKey: presigned.objectKey, fileName: file.name, mimeType: file.type, sizeBytes: file.size }) });
